...
export default function FormPKK({
  ...
  const generateDataPost = (values) => {
    const { trang_thai, ngay_ct, ngay_lap_phieu, ...data } = values;
    const result = {
      ...data,
  ...
  const handleSave = async (values) => {
    ...
    await asyncPostData('pkk', dataPost, method).then((resp) => {
      if (!resp.message) {
        handleClose();
        reset();
        setLoad((prev) => prev + 1);
  ...
  return (
    <ModalBase
      ...
      width="900px"
      title={`${isEdit ? 'Chỉnh sửa' : 'Tạo'} phiếu kiểm kho`}
      actions={[
        <ButtonBase
          key={v4()}
